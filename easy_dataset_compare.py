...
def do_train(trafo_variant, random_state, epochs, N, h, I, d, l_max, learning_rate, warmup_period, eval_ratio, error):
    ...
    if trafo_variant == 'vaswani':
        ...
        labels_train, data_train, mask_train = fetch_data_vaswani(l_max, d_model, h, nlp, 0,
                                                                  easy=True, split='train',
                                                                  random_state=None)
        labels_dev, data_dev, mask_dev = fetch_data_vaswani(l_max, d_model, h, nlp, 0,
                                                            easy=True, split='dev',
                                                            random_state=None)
        labels_test, data_test, mask_test = fetch_data_vaswani(l_max, d_model, h, nlp, 0,
        ...
        labels_train, data_train, mask_train = fetch_data_kohler(l_max, d, h, I, nlp, 0,
                                                                 easy=True, split='train',
                                                                 random_state=None)
        labels_dev, data_dev, mask_dev = fetch_data_kohler(l_max, d, h, I, nlp, 0,
                                                           easy=True, split='dev', random_state=None)
        labels_test, data_test, mask_test = fetch_data_kohler(l_max, d, h, I, nlp, 0,
                                                              easy=True, split='test',
                                                              random_state=None)
    ...
    for epoch in range(epochs):
        ...
        if epoch % eval_ratio == 0:
            ...
            #    break
        if epoch == 1000:
            print("half-time test accuracy: " + str(evaluate(model, labels_test, data_test, mask_test, device)))
...
if __name__ == "__main__":
    ...
    result_vaswani = do_train('vaswani', random_state, epochs, N, h, I, d, l_max, learning_rate, warmup_period,
                              eval_ratio, error)
    result_kohler = do_train('kohler', random_state, epochs, N, h, I, d, l_max, learning_rate, warmup_period,
                             eval_ratio, error)
    ...
    ax2.scatter(np.arange(0, no_evals, eval_ratio), result_vaswani[1], color='blue', label='Vaswani', marker='*')
    ax2.scatter(np.arange(0, no_evals, eval_ratio), result_kohler[1], label='Kohler', marker='o',
                facecolors='none', edgecolors='orange')
    # print("Accuracy on validation set: " + str(final_dev_acc))
    # ax2.scatter([no_evals], [final_dev_acc])
